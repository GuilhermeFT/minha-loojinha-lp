...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-[hsl(var(--border))] bg-[var(--background)]/95 backdrop-blur supports-[backdrop-filter]:bg-[var(--background)]/80">
      ...
      {/* Mobile menu — rendered via portal to avoid the backdrop-filter stacking context of the header */}
      {mounted &&
        mobileMenuOpen &&
        createPortal(
          <>
            ...
            />
            <div
              className="fixed inset-y-0 right-0 z-[200] flex w-full max-w-sm flex-col border-l border-[hsl(var(--border))] bg-[var(--background)] shadow-[var(--shadow-card)]"
              role="dialog"
              aria-modal
